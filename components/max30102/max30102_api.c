...
void read_max30102_fifo(int32_t *red_data, int32_t *ir_data)
{
    ...
    // peter  I2Cdev::readBytes(MAX30102_ADDR, fifo_reg, 6, &un_temp, I2Cdev::readTimeout, wireObj);
...
void read_max30102_reg(uint8_t reg_addr, uint8_t *data_reg, size_t bytes_to_read)
{
	...
	// peter I2Cdev::readBytes(MAX30102_ADDR, reg_addr, bytes_to_read, data_reg, I2Cdev::readTimeout, wireObj);
}
...
void write_max30102_reg(uint8_t command, uint8_t reg)
{
	...
	//uint8_t data[1];
	//data[0] = command;
    //I2Cdev::writeBytes(MAX30102_ADDR, reg, 1, data, wireObj);
}
